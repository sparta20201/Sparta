	...
	<section class="full-box dashboard-contentPage">
		...
		<div class="container-fluid">
			<div class="page-header">
			  <h1 class="text-titles"><i class="zmdi zmdi-book zmdi-hc-fw"></i><?= $user['tipo']; ?><small> - Registrar Pagos</small></h1>
			</div>
        ...
        <!--<table id="customers">-->
          <thead>
            <tr>
              ...
              <?php 
              $records2 = $conn->prepare('SELECT * FROM mensualidad');
              $records2->execute();
              $records3 = $conn->prepare('SELECT * FROM jugador LEFT JOIN mensualidad ON mensualidad.jugador_idjugador');
              $records3->execute();
              while($results2 = $records2->fetch(PDO::FETCH_ASSOC)) { 
                ?>
              <tr class="gradeA">
              	  <td><?php $results3 = $records3->fetch(PDO::FETCH_ASSOC); echo $results3['nombres']," ",$results3['apellidos']; ?></td>
              	  <td><?php echo $results2['estado']; ?></td>
              	  <td>$<?php echo $results2['valor']; ?></td>
                  <td><?php echo $results2['fechapago']; ?></td>
	              <td><?php echo $results2['mensual']; ?></td>
	              <td><img src='<?php echo $results2['imagen']; ?>'height="80" width="120"></td>

				  <td><a href='actualizarpago.php?id=<?php echo $results2['idmensualidad']; ?>' class="btn btn-success btn-raised btn-xs"><i class="zmdi zmdi-refresh"></i></a></td>
				  <td><a href='eliminarpago.php?id=<?php echo $results2['idmensualidad']; ?>' class="btn btn-danger btn-raised btn-xs" onclick="return confirm('¿Estas seguro que desea eliminar el el pago del jugador?\n <?php echo $results3['nombres']," ",$results3['apellidos']; ?>');"><i class="zmdi zmdi-delete"></i></a></td>
              </tr>
               <?php 
               } 
               ?>
