<?php
 ...
 //actualizar mensajes
	if(isset($_POST['fecha'])){
        ...
        $records2=$conn->prepare("UPDATE partido SET fecha=:fecha,hora=:hora,lugar=:lugar,equipo1=:equipo1,equipo2=:equipo2 WHERE idpartido=:idpartido");
        $records2->bindParam(":fecha",$fecha);
        $records2->bindParam(":hora",$hora);
        $records2->bindParam(":lugar",$lugar);
        $records2->bindParam(":equipo1",$equipo1);
        $records2->bindParam(":equipo2",$equipo2);
        $records2->bindParam(":idpartido",$idpartido);
      
	    if($records2->execute()){
	      $message1 = 'Partido actualizado';
	    }else{
	     $message1 = 'No se pudo actualizar el partido';
	    }
	}			
?>
<!DOCTYPE html>
	...
	<section class="full-box dashboard-contentPage">
		...
		<div class="container-fluid">
			<div class="page-header">
			  <h1 class="text-titles"><i class="zmdi zmdi-book zmdi-hc-fw"></i><?= $user['tipo']; ?><small> - Actualizar Partidos</small></h1>
			</div>
			<p class="lead">Espacio para actualizar partidos.
			...
			<div class="row">
				<div class="col-xs-12">
					<ul class="nav nav-tabs" style="margin-bottom: 15px;">
					  	<li class="active"><a href="#new" data-toggle="tab">Actualizar Partido</a></li>
					  	<li><a href="verpartido.php">Volver</a></li>
					</ul>
								...
								<div class="row">
									<div class="col-xs-12 col-md-10 col-md-offset-1">
										...
										//Recuperar datos
										  if(isset($_GET['id'])){
										    $id=$_GET['id'];
										    $records = $conn->prepare("SELECT * FROM partido WHERE idpartido=:idpartido");
										    $records->bindParam(":idpartido",$id);
										    $records->execute();
										    if($records->rowCount()>=1){
										      $results = $records->fetch(PDO::FETCH_ASSOC);
										  ?>
								    <form action="" method="post" enctype="multipart/form-data">
								    	<input name="id" type="hidden" value="<?php echo $results['idpartido']; ?>" required="">
										...
										<div class="form-group label-floating">
										  ...
										  <input name="hora" type="text" value="<?php echo $results['hora']; ?>" class="form-control">
										</div>
										<div class="form-group label-floating">
										  <label class="control-label">Lugar</label>
										  <input name="lugar" type="text" value="<?php echo $results['lugar']; ?>" class="form-control">
										</div>
										<div class="form-group label-floating">
										  <label class="control-label">Equipo1</label>
										  <input name="equipo1" type="text" value="<?php echo $results['equipo1']; ?>" class="form-control">
										</div>
										<div class="form-group label-floating">
										  <label class="control-label">Equipo2</label>
										  <input name="equipo2" type="text" value="<?php echo $results['equipo2']; ?>" class="form-control">
										</div>
									    <p class="text-center">
